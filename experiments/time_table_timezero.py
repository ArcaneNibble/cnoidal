...
output += struct.pack(">qQ", -10, 100)

# pos delta
output += struct.pack(">I", offs[0] + 1)
output += struct.pack(">I", offs[1] - offs[0] - 1)
output += struct.pack(">I", offs[2] - offs[1])

# time delta
output += struct.pack(">Q", 25)
output += struct.pack(">Q", 25)
output += struct.pack(">Q", 25)
...
output += struct.pack(">q", -1000)

#####

# LT_SECTION_EXCLUDE_TABLE
exclude_table_off = len(output)
output += struct.pack(">I", 1)
output += struct.pack(">qQ", -5, 60)
...
output += struct.pack(">IB", time_table_off, 9)         # LT_SECTION_TIME_TABLE64
output += struct.pack(">IB", sync_table_off, 2)         # LT_SECTION_SYNC_TABLE
output += struct.pack(">IB", timezero_off, 20)          # LT_SECTION_TIMEZERO
output += struct.pack(">IB", exclude_table_off, 19)     # LT_SECTION_EXCLUDE_TABLE

output += b'\xB4'   # trlid

with open('time_table_timezero.lxt', 'wb') as f:
    f.write(output)
